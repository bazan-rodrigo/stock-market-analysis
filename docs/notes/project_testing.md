...
description: Existe suite pytest (710 tests de lógica pura); correrla antes de cada push
metadata: 
  node_type: memory
  type: project
...
Desde julio 2026 el proyecto tiene una suite de pytest en `tests/` (710 tests, ~26 s) que cubre la lógica pura: signal_engine, pnf_service, ratios fundamentales, helpers técnicos (fechas, RSI, drawdowns, zonas de régimen/volatilidad, camino rápido del delta, checksum/benchmark staleness, orden de fases rebuild/update), composites anidadas, score de estrategias, soporte/resistencia (sr_service), períodos de retorno (returns_service), normalización RRG (rrg_service) y validación de datos importados (import_service).

Módulo Backtest/Carteras (rediseño jul-2026): `test_portfolio_metrics` (CAGR/Sharpe/Sortino/drawdown/PF/expectancy/matriz mensual), `test_portfolio_views`, `test_portfolio_service` (posiciones por promedio ponderado, parciales, P&L realizado, membresía curada/estrategia), `test_portfolio_sim_engine` (top-N ranking-puro + gated + fixed-weights, sin look-ahead), `test_portfolio_backtest_service` (mapeo trades→barras, cross-section con hueco interior, persistencia de corridas, y walk-forward: ventanas anclado-expansivo, `_span_cagr`, objetivo Sharpe `_wf_score`), `test_rules_backtest_service`. Las funciones que tocan BD (run_portfolio_backtest, walk_forward, _load_raw) se verifican en el Codespace, no en la suite.
