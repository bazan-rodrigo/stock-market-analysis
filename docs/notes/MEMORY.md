...
- [Trinquetes que faltan — 4 huecos medidos](project_trinquetes_faltantes.md) — 1/2-ago. **El patrón: un trinquete sirve si DERIVA del código y se pudre si codifica una lista a mano.** Hechos el #2 (permisos fallan cerrado) y el **#4 completo, que destapó 3 bugs vivos**: el reset a fábrica no vaciaba las tablas anchas, por CLI no vaciaba NADA, y `run_history` no estaba ni en el alcance ni preservada. Toda tabla del esquema debe estar clasificada (alcance o `_PRESERVED_TABLES` con motivo). **Abiertos**: el espejo JS de `simulateTrades` sin ninguna red, nombres de tablas dropeadas en 9 docstrings, el SPEC en PROSA
