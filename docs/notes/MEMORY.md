...
- [Trinquetes que faltan — 4 huecos medidos](project_trinquetes_faltantes.md) — 1/2-ago. **El patrón: un trinquete sirve si DERIVA del código y se pudre si codifica una lista a mano.** Hechos el #2 (permisos fallan cerrado) y medio #4 (el reset a fábrica no vaciaba las tablas anchas, y por CLI no vaciaba NADA). **Abiertos**: el espejo JS de `simulateTrades` sin ninguna red, nombres de tablas dropeadas en 9 docstrings, cobertura derivada de `clean_data`, el SPEC en PROSA
