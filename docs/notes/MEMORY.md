...
- [IA con cuenta propia — MCP: **ANDANDO EN PRODUCCIÓN**](project_ia_mcp.md) — 1/2/3-ago, **16 herramientas de solo lectura**. 3-ago: tenía el CATÁLOGO pero no el CONTRATO (`SPEC.md` solo salía por un botón de descarga) y `search_manual` devolvía **vacío** ante frases enteras. **Una capacidad expuesta a medias no da error, da vacío**. Servicio `mcp` aparte en Railway, migraciones 0099 (token en `users`) + 0100 (OAuth: el conector remoto NO acepta token pegado a mano). Riesgo central resuelto: **el gate de visibilidad no se hereda** (vive en la UI) y se re-aplica en `app/ai`. Ninguna IA escribe señales; backtest y carteras corren **sin persistir**, con KPIs por tramo contra el sobreajuste. Destapó 2 bugs de producción ajenos a la IA. **2-ago: Gemini no conectaba — que ande con Claude no prueba nada sobre otro cliente.** Los clientes de DCR ahora se registran como PÚBLICOS; un fallo de OAuth no dejaba NINGÚN rastro en el log y hubo que interrogar producción a mano
- [La IA mide estrategias que NO existen](project_ia_borrador_estrategias.md) — 3-ago: con 0 estrategias creadas la IA no podía medir nada. Faltaba UN eslabón: el universo (`eligible_by_dates`). **El holdout se EXCLUYE, no se oculta**, y el corte es del calendario, no del pedido. `ic_holdout` pasó a mentir y se renombró
