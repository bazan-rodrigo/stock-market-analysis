...
def seed_reference_data() -> None:
    ...
    existing = s.query(PriceSource).filter(PriceSource.name == "Yahoo Finance").first()
    if existing is None:
        source = PriceSource(
            name="Yahoo Finance",
        ...
        s.add(source)
        s.commit()
        logger.info("Fuente 'Yahoo Finance' creada.")
    else:
        logger.info("Fuente 'Yahoo Finance' ya existe — omitida.")
